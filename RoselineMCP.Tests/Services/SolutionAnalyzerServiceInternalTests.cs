...
public class SolutionAnalyzerServiceInternalTests : IDisposable
{
    ...
    #region IsValidProjectFile

    public class IsValidProjectFileTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_True_When_File_Exists_And_Is_Csproj()
        {
            // Arrange
            var projPath = Path.Combine(_testDirectory, "MyApp.csproj");
            File.WriteAllText(projPath, "<Project />");

            // Act
            var result = InvokePrivate<bool>("IsValidProjectFile", projPath);
        ...
        {
            // Arrange
            var projPath = Path.Combine(_testDirectory, "NonExistent.csproj");
        ...
        [Fact]
        public void Should_Return_False_When_File_Not_Csproj()
        {
            // Arrange
            var projPath = Path.Combine(_testDirectory, "MyApp.txt");
    ...
    #region TryFindProjectInDirectory

    public class TryFindProjectInDirectoryTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_Csproj_When_Present()
        {
            // Arrange
            var projPath = Path.Combine(_testDirectory, "MyApp.csproj");
            File.WriteAllText(projPath, "<Project />");
            ...
            // Act
            var result = InvokePrivate<string?>("TryFindProjectInDirectory", _testDirectory);

            // Assert
            result.ShouldBe(projPath);
        }

        [Fact]
        public void Should_Return_Null_When_No_Csproj_Present()
        {
            // Act
            var result = InvokePrivate<string?>("TryFindProjectInDirectory", _testDirectory);
        ...
        [Fact]
        public void Should_Return_Null_When_Directory_Does_Not_Exist()
        {
            // Arrange
            var nonExistentDir = Path.Combine(_testDirectory, "does-not-exist");

            // Act
            var result = InvokePrivate<string?>("TryFindProjectInDirectory", nonExistentDir);
    ...
    #region GetStartDirectory

    public class GetStartDirectoryTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_Path_When_Directory()
        {
            // Act
            var result = InvokePrivate<string?>("GetStartDirectory", _testDirectory);

            // Assert
            result.ShouldBe(_testDirectory);
        }

        [Fact]
        public void Should_Return_Parent_Dir_When_File_Path()
        {
            // Arrange
            var filePath = Path.Combine(_testDirectory, "some.csproj");
            File.WriteAllText(filePath, "content");

            // Act
            var result = InvokePrivate<string?>("GetStartDirectory", filePath);
    ...
    public class GetParentDirectoryTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_Parent_Directory()
        {
            // Arrange
            ...
            // Act
            var result = InvokePrivate<string?>("GetParentDirectory", subDir);
        ...
        [Fact]
        public void Should_Return_Null_For_Root()
        {
            // Act - root directories have no parent
            var result = InvokePrivate<string?>("GetParentDirectory", "/");

            // Assert - may be null or empty depending on OS
            // Just ensure it doesn't throw
            // On Linux, "/" returns null parent
        }
    }

    #endregion

    #region TryFindSolutionInDirectory

    public class TryFindSolutionInDirectoryTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_Sln_When_Present()
        {
            // Arrange
            ...
            // Act
            var result = InvokePrivate<string?>("TryFindSolutionInDirectory", _testDirectory);
        ...
        [Fact]
        public void Should_Return_Null_When_No_Sln()
        {
            // Act
            var result = InvokePrivate<string?>("TryFindSolutionInDirectory", _testDirectory);
    ...
    #region FindSolutionFile

    public class FindSolutionFileTests : SolutionAnalyzerServiceInternalTests
    {
        [Fact]
        public void Should_Return_Sln_In_Parent_Directory()
        {
            // Arrange
            ...
            // Act - pass the subdir (which has no .sln), should find it in parent
            var result = InvokePrivate<string?>("FindSolutionFile", subDir);
        ...
        [Fact]
        public void Should_Return_Null_When_No_Sln_Anywhere()
        {
            // Arrange - create an isolated temp dir with no .sln
            var isolatedDir = Path.Combine(Path.GetTempPath(), $"Isolated_{Guid.NewGuid()}");
            Directory.CreateDirectory(isolatedDir);

            try
            {
                // This should search up until it can't find any .sln
                // We can't guarantee this returns null on CI (might find one up the tree)
                // So just test it doesn't throw
                var result = InvokePrivate<string?>("FindSolutionFile", isolatedDir);
                // result can be null or a path (if there's a .sln up the directory tree)
            }
            finally
            {
