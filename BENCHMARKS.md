...
truth** — `RoselineMCP.TokenBenchmark` generates it. `index.astro` and `benchmark.astro` compute
**the headline median itself** from that file (`Math.round(x * 100)`), so that one number cannot
drift on those pages. Every other published statement of it is hand-written, and
`RoselineMCP.Tests/Release/MedianFigureContractTests.cs` pins each one against the JSON:
`README.md` (four sites, badge included), this file, `docs/AGENT-BENCHMARK.md` (both occurrences),
`website/src/layouts/Base.astro` (both), `mcpb/manifest.json` and `website/og-card.html`.
...
stale PNG still passes. The image is regenerated by hand from that template (the procedure is in
`og-card.html`'s header comment), and `Base.astro`'s `og:image:alt` is the only machine-readable
claim about what it shows. So when the median changes, re-render the card as a deliberate step: no
test will tell you that you forgot. A CI step that re-renders the template and diffs it against the
committed PNG would close this mechanically and is worth doing; it is out of scope here only
because it adds a build-time render this change deliberately does not introduce.
