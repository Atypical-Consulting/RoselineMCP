...
### Where the headline median is published, and what checks it

`website/src/data/benchmark-results.json` (`headline.medianSavingsReadTools`) is the **source of
truth** — `RoselineMCP.TokenBenchmark` generates it. The site's `index.astro` and `benchmark.astro`
compute their figure from it and therefore cannot drift. Every other published statement of that
number is hand-written, and `RoselineMCP.Tests/Release/MedianFigureContractTests.cs` pins each one
against the JSON: `README.md` (four sites, badge included), this file, `docs/AGENT-BENCHMARK.md`
(both occurrences), `website/src/layouts/Base.astro` (both), `mcpb/manifest.json` and
`website/og-card.html`. Re-measuring is meant to fail all of them at once — update the surfaces the
failure names; do not loosen the test.

⚠️ **One gap the test cannot close: `website/public/og.png`.** The card is a rendered image, so the
guard pins its **template** (`og-card.html`) and not its pixels — a corrected template with a stale
PNG still passes. The image is regenerated by hand from that template (the procedure is in
`og-card.html`'s header comment), and `Base.astro`'s `og:image:alt` is the only machine-readable
claim about what it shows. When the median changes, re-render the card as a deliberate step; nothing
will tell you that you forgot.
